...
async def lifespan(app: FastAPI):
    ...
    logger.info("api_startup", env=settings.app_env, version="0.3.0")
    ...
    await ch.connect(); await ch.init_schema(); await ch.close()
...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(title="CloudSense API", description="FinOps Multi-Agent Platform — FOCUS 1.0 compliant",
                  version="0.3.0", docs_url="/docs", redoc_url="/redoc", lifespan=lifespan)
    app.add_middleware(CORSMiddleware, allow_origins=["*"] if settings.debug else ["https://cloudsense.io"],
    ...
    async def health() -> dict:
        return {"status": "ok", "service": "cloudsense-api", "version": "0.3.0"}
